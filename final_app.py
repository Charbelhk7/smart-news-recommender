...
if submitted:
    ...
    for i, row in videos_df.iterrows():
        ...
        if match_count > 0:
            matched_values = {
                k: v for k, v in {
                    'Tone': ", ".join(user_clean['tone']) if 'Tone' in match_reasons else "",
                    'Style': ", ".join(user_clean['style']) if 'Style' in match_reasons else "",
                    'Emotion': ", ".join(user_clean['emotion']) if 'Emotion' in match_reasons else "",
                    'Region': ", ".join(user_clean['region_tags']) if 'Region' in match_reasons or 'Region (anywhere)' in match_reasons else "",
                    'Duration': ", ".join(user_clean['duration_group']) if 'Duration' in match_reasons or 'Any duration' in match_reasons else "",
                    'Language': user_clean['language'].capitalize() if 'Language' in match_reasons else "",
                    'Topic': ", ".join(user_clean['category']) if 'Topic' in match_reasons else ""
                }.items() if v
            }
            ...
            matched_videos.append({
                ...
                'unmatched_fields': [
                    f.capitalize() for f in ['tone', 'style', 'emotion', 'topic']
                    if f not in [reason.lower() for reason in match_reasons]
                ]
            })
...
if st.session_state.last_results:
    ...
    for vid in st.session_state.last_results:
        ...
        st.write("DEBUG matched_values:", matched_values)
