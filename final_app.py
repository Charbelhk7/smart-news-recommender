...
if submitted:
    ...
    for i, row in videos_df.iterrows():
        ...
        if match_count > 0:
            ...
            # Define mapping between internal field names and display names
            feature_mapping = {
                'tone': 'Tone',
                'style': 'Style',
                'emotion': 'Emotion',
            ...
            for internal_key, display_name in feature_mapping.items():
                # Skip if not in match_reasons (use lowercase for safety)
                if display_name.lower() not in [r.lower() for r in match_reasons]:
                    continue

                # Pull user-facing label(s) from original input
                original_value = user_original.get(internal_key)

                # Handle strings vs lists
                if isinstance(original_value, list):
                    if original_value:
                        matched_values[display_name] = ", ".join(original_value)
                elif isinstance(original_value, str):
                    if original_value.strip():
                        matched_values[display_name] = original_value.strip()
            ...
            matched_videos.append({
                ...
                'unmatched_fields': [
                    ...
                    if user_clean.get(feature_key)
                    and display_name.lower() not in [reason.lower() for reason in match_reasons]
                ]
